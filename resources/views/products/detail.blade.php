        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            @if($product)
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
                    ...
                    <div class="space-y-4">
                        <!-- Main Image -->
                        <div class="bg-background rounded-2xl shadow-lg overflow-hidden border border-border">
                    ...
                    <div class="space-y-8">
                        ...
                        <div>
                            <div class="flex items-center gap-3 mb-4">
                                @if($product->category)
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-primary text-accent">
                                        {{ $product->category->nama_kategori }}
                                    </span>
                                @endif
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {{ $product->tipe_produk === 'custom' ? 'bg-accent text-text border border-border' : 'bg-primary text-accent' }}">
                                ...
                                <div class="grid grid-cols-4 gap-3 mb-6" id="size-buttons">
                                    @foreach($product->variants as $variant)
                                        <button @click="selectSize('{{ $variant->size }}', {{ $variant->id }}, {{ $variant->stock }}, {{ $variant->price_adjustment }})"
                                                class="size-btn border-2 px-4 py-3 rounded-xl transition-all font-medium {{ $loop->first ? 'border-primary text-primary bg-accent' : 'border-secondary text-text' }} {{ $variant->stock === 0 ? 'opacity-50 cursor-not-allowed bg-gray-100' : 'hover:border-primary hover:text-primary' }}"
                                                {{ $variant->stock === 0 ? 'disabled' : '' }}>
                                            <div class="text-center">
                                                <div class="font-semibold">{{ $variant->size }}</div>
                                                <div class="text-xs mt-1 {{ $variant->stock === 0 ? 'text-gray-500' : 'text-textSecondary opacity-70' }}">Stok: {{ $variant->stock }}</div>
                                            </div>
                                        </button>
                                    @endforeach
                                </div>
                        ...
                        <div class="space-y-4">
                            <div class="flex gap-4">
                                <button @click="addToCart({{ $product->id }}, selectedVariantId, {{ auth()->check() ? 'true' : 'false' }})"
                                        class="flex-1 bg-primary text-accent px-8 py-4 rounded-xl font-semibold hover:bg-secondary transition-all transform hover:scale-105 shadow-lg">
                        ...
                        <div class="mt-8">
                            ...
                            <div class="text-textSecondary leading-relaxed">
                        ...
                        <div class="mt-6">
                            ...
                            <div class="grid grid-cols-1 gap-4">
                                <div class="flex justify-between py-2 border-b border-border">
                                    <span class="text-textSecondary opacity-80">Kategori</span>
                                    <span class="text-text font-medium">{{ $product->category->nama_kategori ?? 'Tidak dikategorikan' }}</span>
                                </div>
                                <div class="flex justify-between py-2 border-b border-border">
                                    <span class="text-textSecondary opacity-80">Tipe Produk</span>
            ...
            @else
                <div class="text-center py-20">
                    <div class="w-24 h-24 bg-accent rounded-full flex items-center justify-center mx-auto mb-6 border border-border">
                    ...
                    <p class="text-textSecondary mb-8 opacity-80">Produk yang Anda cari tidak tersedia atau telah dihapus.</p>
                    <div class="flex gap-4 justify-center">
                        <a href="/" class="bg-primary text-accent px-6 py-3 rounded-lg font-semibold hover:bg-secondary transition-colors">
                            Kembali ke Beranda
                        </a>
                        <a href="/products" class="border border-border text-text px-6 py-3 rounded-lg font-semibold hover:bg-accent transition-colors">
                            Lihat Produk Lain
                        </a>
                    </div>
                </div>
            ...
            return {
                ...
                selectSize(size, variantId, stock, priceAdjustment) {
                    ...
                    document.querySelectorAll('.size-btn').forEach(btn => {
                        ...
                        btn.classList.add('border-secondary', 'text-text');
                        btn.classList.remove('bg-accent');
                    });

                    event.target.closest('.size-btn').classList.remove('border-secondary', 'text-text');
