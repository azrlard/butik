<nav x-data="{ mobileMenuOpen: false }" class="bg-white/95 backdrop-blur-sm shadow-lg border-b border-gray-100 fixed top-0 left-0 right-0 z-50">
        ...
        <div class="flex justify-between items-center h-16">
            ...
            <div class="hidden md:flex items-center space-x-1">
                ...
                <a href="/cart" class="text-gray-700 hover:text-indigo-600 px-4 py-2 rounded-lg text-sm font-medium transition-colors hover:bg-indigo-50 flex items-center">
                    Keranjang
                    <span x-data="{ cartCount: 0 }" x-init="cartCount = JSON.parse(localStorage.getItem('cart') || '[]').reduce((sum, item) => sum + item.quantity, 0)" x-text="cartCount" class="ml-2 bg-yellow-400 text-xs rounded-full h-5 w-5 flex items-center justify-center text-white font-semibold">0</span>
                </a>
                @if(auth()->check())
                    <div x-data="{ userMenuOpen: false }" class="relative">
                        <button @click="userMenuOpen = !userMenuOpen" class="text-gray-700 hover:text-indigo-600 px-4 py-2 rounded-lg text-sm font-medium transition-colors hover:bg-indigo-50 flex items-center">
                            {{ auth()->user()->name }}
                        </button>

                        <div x-show="userMenuOpen" x-cloak @click.away="userMenuOpen = false" class="absolute right-0 mt-2 w-56 bg-white rounded-lg shadow-lg border border-gray-100 py-2 z-50">
                            <a href="/profile" @click="userMenuOpen = false" class="flex items-center px-4 py-3 text-sm text-gray-700 hover:bg-indigo-50 hover:text-indigo-600 transition-colors">
                                Informasi Akun
                            </a>
                            <a href="/orders" @click="userMenuOpen = false" class="flex items-center px-4 py-3 text-sm text-gray-700 hover:bg-indigo-50 hover:text-indigo-600 transition-colors">
                                Riwayat Pesanan
                            </a>
                            <a href="/settings" @click="userMenuOpen = false" class="flex items-center px-4 py-3 text-sm text-gray-700 hover:bg-indigo-50 hover:text-indigo-600 transition-colors">
                                Pengaturan
                            </a>
    ...
    <div x-show="mobileMenuOpen" x-cloak class="hidden md:hidden bg-white/95 backdrop-blur-sm border-t border-gray-100 shadow-lg">
